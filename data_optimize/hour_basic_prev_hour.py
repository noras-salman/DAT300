...
output_file_name='_hour_basic_prev_hour'
    ...
    if  accept_condition: #condition of feautre selection
      ...
      if hour_based:	  
        ...
        if current_hour==int(hour):
          ...
          if active_count>1:
            file_output.write(str(day_of_week_update)+ ','+ str(current_hour+1)+ ','+ str(value[active_count-2])+ ','+str(hour_load)+'\n') # The final ourput (add +1 the current hour to avoid zero value)
          current_hour=int(hour)
          hour_load=sum       
      
      else:  #minute based
        active_count+=1
        file_output.write(str(day_of_week)+ ','+ str(minute)+ ','+str(sum)+'\n') # The final output the will be written to the file :: python will convert \n to os.linesep
